...
data_path = 'C:/Schuster Lab/Multimode/GRAPE-Tensorflow-Examples-master/pulses/output_pulses/'
...
steps = 500
#Defining H0
qubit_state_num = 4  # changed from 4 to 2 to test code faster
...
mode_state_num = 6 # changed from 5 to 3 to test code faster
chi = -0.565e-3
kappa = -chi / 10.0
...
H0 = np.kron(I_q, H_m) + 2 * 2 * np.pi * chi * np.kron(Q_z, M_z)
...
Hnames = []
controlHs = {}
# separate drive operators for each transmon ge, ef, fh, etc. transition
for state in range(qubit_state_num - 1):
    ...
    controlHs["X" + str(state)] = np.kron(piX, I_m)
    controlHs["Y" + str(state)] = np.kron(piY, I_m)
    Hops.extend([controlHs['X' + str(state)], controlHs['Y' + str(state)]])
    ops_max_amp.extend([0.05*2*np.pi, 0.05*2*np.pi])
    Hnames.extend(["xi" + str(state), "yi" + str(state)])
Hops.extend([IX, IY])
ops_max_amp.extend([0.01*2*np.pi, 0.01*2*np.pi])
Hnames.extend(['ix','iy'])
...
ss = Grape(H0, Hops, Hnames, U, total_time, steps, psi0, convergence=convergence,
                     ...
                     maxA=ops_max_amp, reg_coeffs=reg_coeffs, dressed_info=dressed_info, file_name="binomial_code_noAlpha",
